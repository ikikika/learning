# Quickstart: Validate shell add-remote
...
- Two terminals available (shell + at least one remote), or reuse compose harness patterns
...
npm run add-remote -- --alias=demoRemote --name=demoRemote --port=3002 --props='{"title":"From Shell A"}'
```
...
Restart shell (`npm start`), open shell, click remote nav → panel shows Route 1 and visible host title **From Shell A** (`demo-remote-host-title` or equivalent).
...
Edit `starter.role.json` → `remoteProps.demoRemote.title` to a new string; restart shell; reopen demo remote → new title visible without re-add.

## 5. Stale props ignored

Add `remoteProps.orphanAlias: { "title": "x" }` without registering `orphanAlias`; shell still starts; registered remotes load.

## 6. Internal routes

With remote open in panel, exercise remote-owned Route 2 (MemoryRouter / remote-internal navigation as implemented) → content switches; shell chrome remains.
...
npm run test:e2e   # or project scripts for standalone / remote-standalone / shell
```
