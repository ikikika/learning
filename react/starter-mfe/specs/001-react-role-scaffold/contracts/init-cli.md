...
node scripts/init.mjs --role=<standalone|host|remote> --port=<number> [--name=<appName>] [--remote=alias:name[:expose[:urlEnv]]]... [--remote-name=<remoteAppName>] [--force]
```
...
|------|--------|----------|
| `--role` | `standalone`, `host`, `remote` | Yes in non-TTY; prompted when stdin is a TTY |
...
| `--remote` | `alias:name[:expose[:urlEnv]]` (repeatable) | No — **host only**. Default expose is PascalCase of `name` (e.g. `demoRemote:checkout` → `./Checkout`). When omitted (and no `--remote-name`), `remotes[]` is **empty** |
| `--remote-name` | same rules as `--name` | No — **host only**; shorthand for a single `demoRemote:<name>` entry. Mutually exclusive with `--remote` |
...
1. **Role** — numbered list (`1–3`) or type `standalone` / `host` / `remote`
...
|-----------|------|---------------------|
| Missing `--role` (non-TTY) | non-zero | `--role=standalone\|host\|remote` required |
...
| `--remote` / `--remote-name` without `--role=host` | non-zero | Only valid with host |
...
| Success | 0 | Writes metadata; updates README + `.env` port; host regenerates loaders |

## Side effects on success

1. Write/overwrite `starter.role.json` ([role-metadata.schema.json](./role-metadata.schema.json)), including `name`, `federationName`, (remote) `expose`, and (host) `remotes[]`.
2. Update README role + app name + start instructions.
3. Set the role’s port in `.env` (`PORT_STANDALONE` / `PORT_HOST` / `PORT_REMOTE`). `.env.example` leaves these blank; init creates `.env` from the example when missing and fills the active role’s key.
...
6. Host init regenerates `src/app/remotes/loaders.generated.ts` with a static `import()` per remote alias (empty registry when `remotes[]` is empty).
...
| `remote` | Webpack exposes PascalCase name → `./src/app/App.tsx`; prints a copy-paste `npm run add-remote` command for the host |
