...
Applies when role is `remote` (and when a host consumes this remote).
...
Demo and host sample assets both live under `src/` permanently. Init does not
prune demo source when the role is host (webpack/routes select the active
...
2. `init --role=host --port=… --remote=demoRemote:demoRemote` in one; `init --role=remote --port=…` in the other.
3. Start both; Playwright asserts host owns PWA install/offline UX and document
   `data-theme`; embedded remote does not take over either.
4. Host mounts with `embedded={true}`.

## Rename

Changing `--name` (re-init with `--force`) updates `expose` / host `remotes[]`
snippet. Keep Webpack exposes, host remotes map, and docs aligned.
